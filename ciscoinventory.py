...
if __name__ == '__main__':
    stoidmodel = "1.3.6.1.2.1.47.1.1.1.1.13"
    snoid = "1.3.6.1.2.1.47.1.1.1.1.11"
    sysnameoid = "1.3.6.1.2.1.1.5"
    syslocationoid = "1.3.6.1.2.1.1.6"
    ...
    parcer.add_argument('-C', type=str, help="Context", default="")
    ...
    for ipa in ipaddrslist:
        ...
        rtname = GoWithSnmp1.nextoiddata(ipa, sysnameoid)
        if rtname == None:
            print("No Response!")
            ...
            rtsysdescription = GoWithSnmp1.nextoiddata(ipa, sysdescroid, True)
            ...
            rtsyslocation = GoWithSnmp1.nextoiddata(ipa, syslocationoid, True)
            ...
            if DEBUGGPRINT:
                print(rtname[1] + "\r\n" + "\r\n" + rtsyslocation[1])
            rt1 = GoWithSnmp1.walkoiddata(ipa,stoidmodel,False)
            if rt1 != None:
                for el in rt1:
                    for name, val in el:
                        #print(name.prettyPrint()+" "+val.prettyPrint())
                        if len(val.prettyPrint()) > 4:
                            vrids = name.prettyPrint()
                            vridl = vrids.split('.')
                            vrid = vridl[len(vridl)-1]
                            #print(vrid)
                            mkoidsn = snoid+"."+vrid
                            rt2 = GoWithSnmp1.getoiddata(ipa,mkoidsn)
                            if rt2 != None:
                                print(val.prettyPrint() + ";" + rt2[1])
                                invrec = ET.SubElement(sinv,"invrec")
                                invrecin = ET.SubElement(invrec,"modulename").text = val.prettyPrint()
                                invrecin2 = ET.SubElement(invrec,"SN").text = rt2[1]
